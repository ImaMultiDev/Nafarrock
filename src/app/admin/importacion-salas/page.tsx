...
import type { VenueImportRow, RowValidationError } from "@/lib/csv-venues";

type Step = "upload" | "preview" | "done";

export default function ImportacionSalasPage() {
  const [step, setStep] = useState<Step>("upload");
  const [valid, setValid] = useState<VenueImportRow[]>([]);
  ...
  const handleDownloadTemplate = () => {
    window.open("/api/admin/venues/import/template", "_blank");
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/venues/import/validate", {
        method: "POST",
        body: formData,
      });
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 font-body text-punk-white/60">
            {step === "upload" && "Sube un CSV para validar e importar espacios"}
            {step === "preview" && `${valid.length} espacios listos para importar`}
        ...
        <div className="mt-10 max-w-2xl space-y-6">
          ...
          <div className="border-2 border-punk-white/20 bg-punk-black/60 p-6">
            ...
            <p className="mt-2 font-body text-sm text-punk-white/60">
              Columnas: Nombre, Ciudad, Dirección, Descripción, Web(URL), Maps(URL),
              Instagram(URL), Facebook(URL). Mínimo: Nombre y Ciudad.
            </p>
            <p className="mt-1 font-body text-xs text-punk-white/50">
              Podrás añadir imágenes, categoría y demás datos individualmente después.
            </p>
        ...
        <div className="mt-10 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <p className="font-body text-punk-white/80">
              {valid.length} espacios válidos. Se crearán como{" "}
              ...
              <tbody>
                {valid.map((row, i) => (
                  <tr key={i} className="border-b border-punk-white/10">
                    ...
                    <td className="py-3 font-body text-sm text-punk-white/60 max-w-[200px] truncate">
                      {row.direccion || "—"}
                    </td>
                    <td className="py-3 font-body text-sm text-punk-white/60 max-w-[150px] truncate">
                      {row.web_url || "—"}
                    </td>
                  </tr>
                ))}
        ...
        <div className="mt-10 max-w-2xl space-y-6">
          <div className="border-2 border-punk-pink/50 bg-punk-pink/10 p-6">
            ...
            <p className="mt-2 font-body text-punk-white/90">
              {importResult.created} espacios creados correctamente.
