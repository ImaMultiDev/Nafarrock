...
import { BandasOptimizedFilters } from "./BandasOptimizedFilters";
import { BandasOptimizedCard } from "./BandasOptimizedCard";
...
async function fetchBandas(
  ...
  location: string,
  search: string
): Promise<{ items: BandItem[]; total: number; hasMore: boolean }> {
  const params = new URLSearchParams();
  params.set("page", String(page));
  params.set("pageSize", String(PAGE_SIZE));
  if (location) params.set("location", location);
  if (search.trim()) params.set("search", search.trim());
  const res = await fetch(`/api/bandas?${params.toString()}`);
  if (!res.ok) throw new Error("Error al cargar bandas");
  return res.json();
}

export function BandasOptimizedView() {
  const t = useTranslations("bands");
  const tActions = useTranslations("common.actions");
  const tFilters = useTranslations("filters.bandas");
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries({ queryKey: ["bandas-optimized"] });
    };
  }, [queryClient]);
  ...
  const {
    ...
    queryKey: ["bandas-optimized", location, submittedSearch],
    queryFn: ({ pageParam }) => fetchBandas(pageParam, location, submittedSearch),
    initialPageParam: 1,
  ...
  const allBands = data?.pages.flatMap((p) => p.items) ?? [];
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-punk-black">
      ...
      />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_0%,rgba(0,200,83,0.08)_0%,transparent_50%)]" />
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        {!headerVisible && allBands.length > 0 && (
          <button
            type="button"
            onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
            aria-label={tFilters("search")}
            className="fixed bottom-[max(1.5rem,env(safe-area-inset-bottom))] right-[max(1.5rem,env(safe-area-inset-right))] z-30 flex items-center gap-2 rounded-full border-2 border-punk-green bg-punk-green px-4 py-3 font-punch text-xs uppercase tracking-widest text-punk-black shadow-lg transition-all hover:bg-transparent hover:text-punk-green"
          >
          ...
          <div className="mx-auto max-w-7xl 2xl:max-w-content-wide">
            {isLoading ? (
              ...
              <div className="rounded-xl border-2 border-punk-white/20 border-dashed p-12 text-center">
                <p className="font-body text-punk-white/60">{t("error") || "Error al cargar bandas"}</p>
              </div>
            ) : allBands.length === 0 ? (
              <div className="rounded-xl border-2 border-punk-white/20 border-dashed p-12 text-center">
              ...
              <>
                <div className="grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
                  {allBands.map((band) => (
                    <BandasOptimizedCard key={band.id} band={band} />
                  ))}
                </div>
                ...
                {hasNextPage && (
                  <div ref={loadMoreRef} className="flex justify-center py-8">
                    <button
                      ...
                      className="rounded-lg border-2 border-punk-green/50 bg-punk-green/10 px-6 py-3 font-punch text-xs uppercase tracking-widest text-punk-green transition-colors hover:bg-punk-green/20 disabled:opacity-50"
                    >
                      {isFetchingNextPage ? (
