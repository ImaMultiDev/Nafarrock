...
type Props = {
  entity: "band" | "venue" | "event" | "promoter" | "organizer" | "festival" | "association" | "boardAnnouncement";
  id: string;
  approved: boolean;
};
...
export function ApproveButton({ entity, id, approved }: Props) {
  ...
  const handleClick = async () => {
    setLoading(true);
    try {
      const paths: Record<string, string> = { band: "bands", venue: "venues", event: "events", promoter: "promoters", organizer: "organizers", festival: "festivals", association: "associations", boardAnnouncement: "tablon" };
