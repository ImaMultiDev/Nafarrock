...
import { SalasOptimizedFilters, type CategoryValue } from "./SalasOptimizedFilters";
...
function VenueCard({ venue }: { venue: VenueItem }) {
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-xl border-2 border-punk-white/10 bg-punk-black transition-all duration-300 hover:border-punk-pink/50 hover:shadow-[0_0_24px_rgba(255,0,110,0.12)] active:scale-[0.99]"
    >
      <div className="aspect-[16/10] overflow-hidden bg-punk-black">
        {venue.logoUrl || venue.imageUrl || (venue.images && venue.images[0]) ? (
          ...
          <div className="flex h-full w-full items-center justify-center font-display text-5xl text-punk-pink/30">
            {venue.name.charAt(0)}
          </div>
        )}
      </div>
      <div className="flex flex-1 flex-col p-4">
        <h2 className="font-display text-lg font-semibold tracking-tight text-punk-white transition-colors group-hover:text-punk-pink line-clamp-2">
          {venue.name}
        </h2>
        {venue.city && (
          <p className="mt-1 font-body text-sm text-punk-white/60">{venue.city}</p>
        )}
        {venue.capacity && (
...
export function SalasOptimizedView() {
  const t = useTranslations("scene.venues");
  const tFilters = useTranslations("filters.salas");
  ...
  useEffect(() => {
    return () => {
      queryClient.removeQueries({ queryKey: ["salas-optimized"] });
    };
  }, [queryClient]);
  ...
  const {
    ...
    queryKey: ["salas-optimized", category, submittedSearch],
    queryFn: ({ pageParam }) => fetchSalas(pageParam, category, submittedSearch),
    initialPageParam: 1,
  ...
  const allVenues = data?.pages.flatMap((p) => p.items) ?? [];
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-punk-black">
      ...
      />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_60%_50%_at_50%_0%,rgba(255,0,110,0.08)_0%,transparent_50%)]" />
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        {!headerVisible && allVenues.length > 0 && (
          <button
            type="button"
          ...
          <div className="mx-auto max-w-7xl 2xl:max-w-content-wide">
            {isLoading ? (
              ...
              <div className="rounded-xl border-2 border-punk-white/20 border-dashed p-12 text-center">
                ...
                <Link
                  href="/escena"
                  className="mt-4 inline-block font-punch text-sm uppercase tracking-widest text-punk-pink transition-colors hover:text-punk-pink/80"
                >
                  ← Volver a Escena
                </Link>
              </div>
                ...
                {hasNextPage && (
                  <div ref={loadMoreRef} className="flex justify-center py-8">
                    <button
                      ...
                      className="rounded-lg border-2 border-punk-pink/50 bg-punk-pink/10 px-6 py-3 font-punch text-xs uppercase tracking-widest text-punk-pink transition-colors hover:bg-punk-pink/20 disabled:opacity-50"
                    >
                      {isFetchingNextPage ? (
