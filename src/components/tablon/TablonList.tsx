...
function AnnouncementCard({ a, t }: { a: Announcement; t: (key: string) => string }) {
  ...
  return (
    <motion.article
      variants={item}
      className="overflow-hidden border-2 border-punk-yellow/50 bg-punk-black transition-all duration-300 hover:border-punk-yellow/80 hover:shadow-[0_0_30px_rgba(255,214,10,0.1)]"
    >
      {/* Imágenes tipo feed: principal grande + galería pequeña */}
      {hasImages && (
        <div className="relative border-b-2 border-punk-yellow/20">
          {mainImage ? (
      ...
      <div className="p-5 sm:p-6">
        ...
        <p className="mt-3 whitespace-pre-wrap font-body text-punk-white/80">
          {a.description}
        </p>
        <a
          href={`mailto:${a.contactEmail}`}
          className="mt-4 inline-block font-punch text-xs uppercase tracking-widest text-punk-yellow transition-colors hover:text-punk-yellow/80"
        >
